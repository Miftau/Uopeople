...
public class DrawTextItem {
	...
	private ArrayList<DrawTextItem> textItems = new ArrayList<>();  // The String that is drawn by this item.
	 ...
	 */
	public DrawTextItem(ArrayList<DrawTextItem> stringToDraw) {
	 ...
	 * @throws NullPointerException if the parameter is null
	 */
	public DrawTextItem(ArrayList<DrawTextItem> stringToDraw, int x, int y) {
		if (stringToDraw == null)
			throw new NullPointerException("String can't be null.");
		textItems = stringToDraw;
		this.x = x;
		this.y = y;
	}
	
	public DrawTextItem(String text, int x2, int y2) {
	...
	public void draw(Graphics g) {
		...
		g2.drawString(toString(),-width/2, -height/2 + fm.getAscent());
	}
	 ...
	 */
	public ArrayList<DrawTextItem> getString() {
		return textItems;
	}
